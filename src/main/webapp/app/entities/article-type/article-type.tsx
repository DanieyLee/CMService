...
import { getEntities, reset } from './article-type.reducer';
import { IArticleType } from 'app/shared/model/article-type.model';
import { APP_DATE_FORMAT, APP_LOCAL_DATE_FORMAT } from 'app/config/constants';
import { ITEMS_PER_PAGE } from 'app/shared/util/pagination.constants';
import { overridePaginationStateWithQueryParams } from 'app/shared/util/entity-utils';

export interface IArticleTypeProps extends StateProps, DispatchProps, RouteComponentProps<{ url: string }> {}

export const ArticleType = (props: IArticleTypeProps) => {
            ...
            <Table responsive>
              ...
              <tbody>
                {articleTypeList.map((articleType, i) => (
                  <tr key={`entity-${i}`}>
                    <td>
                      <Button tag={Link} to={`${match.url}/${articleType.id}`} color="link" size="sm">
                        {articleType.id}
                      </Button>
                    </td>
                    ...
                    <td>
                      {articleType.creatTime ? <TextFormat type="date" value={articleType.creatTime} format={APP_DATE_FORMAT} /> : null}
                    </td>
                    <td>{articleType.updateUser}</td>
                    <td>
                      {articleType.updateTime ? <TextFormat type="date" value={articleType.updateTime} format={APP_DATE_FORMAT} /> : null}
                    </td>
                    <td>{articleType.note}</td>
                    <td className="text-right">
                      <div className="btn-group flex-btn-group-container">
                        <Button tag={Link} to={`${match.url}/${articleType.id}`} color="info" size="sm">
